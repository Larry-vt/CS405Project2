...
public class MemoryAllocator {
	...
	ArrayList<Process> all = new ArrayList<Process>();// every process that'll run in order next process always kept at
														// zero
	...
	public void firstfit() {
		...
		while (total_Processes != done.size()) {// while not all process are complete
			compact();
			// check for holes check left of every index in running in current is hole and
			...
			if (!all.isEmpty() && fits(all.get(0).size)) {// if current waiting process can fit
				for (int i = 0; i < running.size(); i++) {
					if (running.get(i).name.equals("free") && running.get(i).size >= all.get(0).size) { // if we have
																										// free space
																										// add
						if (running.get(i).size - all.get(0).size != 0) {
							Process newHole = new Process(running.get(i).size - all.get(0).size, -1, "free"); // new
																												// hole
							running.set(i, all.get(0));// set where the hole was wiht new process
							running.add(1, newHole);// add hole next to old self
							all.remove(0);
							change = true;
							break;
